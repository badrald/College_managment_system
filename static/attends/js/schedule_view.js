...
jQuery(document).ready(function ($) {
  ...
  SchedulePlan.prototype.placeEvents = function () {
    ...
    this.singleEvents.each(function () {
      ...
      var start = getScheduleTimestamp($(this).attr("data-start")),
        duration = getScheduleTimestamp($(this).attr("data-end"));
      duration = (duration > 1020 ? 1050 : duration) - start;
      var eventTop =
  ...
  SchedulePlan.prototype.openModal = function (event) {
    ...
    $.ajax({
      url: "/attendess/Schedule/view",
      method: "GET",
      dataType: "Json",
      data: {
        "event-content": event.parent().attr("data-content").split("-")[1],
      },
      success: function (data) {
        ...
        // Once the event content has been loaded, add the "content-loaded" class
        self.element.addClass("content-loaded");
      },
      error: function () {
        // Handle errors if the AJAX request fails
        console.error("خطأ في تحميل العناصر للوردية");
      },
    });
  ...
  // Get all the schedule elements with class 'single-event'
  var scheduleElements = document.querySelectorAll(".single-event");

  // Loop through each schedule element and set a random data-event attribute
  scheduleElements.forEach(function (scheduleElement) {
    var randomNumber = getRandomNumber(1, 4);
    scheduleElement.setAttribute("data-event", "event-" + randomNumber);
  });
