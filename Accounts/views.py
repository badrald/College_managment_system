    ...
    if request.method == "POST":
        ...
        if user is None:
            if request.headers.get("x-requested-with") == "XMLHttpRequest":
                return JsonResponse(
                    {"error": "اسم المستخدم او كلمة المرور غير صحيحة"}, status=400
                )
        else:
            login(request, user)
            if request.headers.get("x-requested-with") == "XMLHttpRequest":
...
def profile(request, id):
    if id:
        profile = get_object_or_404(request.user, id=id)
...
def profile_edit(request, id):
    ...
    if (
        request.user.profile.job_place.id == 1
        or profile.job_place == request.user.profile.job_place
    ):
        if request.method == "POST" and "permissions" not in request.POST:
            ...
            if formProfile.is_valid() and formUser.is_valid():
                ...
                myprofile.save()
                return JsonResponse(
                    {"success": "تم تحديث بيانات الحساب بنجاح"}, status=200
                )
    ...
    try:
        if request.method == "POST":
            if request.user == user_delete:
                return JsonResponse(
                    {"error": "هذا المستخدم قيد الاستعمال حاليا , تعذر عملية الحذف "},
                    status=400,
                )
            else:
                user_delete.delete()
                return JsonResponse(
                    {"success": "تمت عملية حذف المستخدم بنجاح"}, status=200
                )

    except:
        return JsonResponse({"error": "هناك خطأ في الفورم"}, status=500)
    return render(
        request, "management/accounts/delete_user.html", {"user": user_delete}
    )
...
def profiles_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if profile_type == "student":
                ...
                MediaProfile.objects.create(user=user)
            return JsonResponse({"success": "تمت عملية إضافة المستخدم بنجاح"})
        else:
    ...
    if user_type == "student":
        ...
        profiles = LecturerProfile.objects.all()
    elif user_type == "staff":
        profiles = StaffProfile.objects.all()
    elif user_type == "media":
        profiles = MediaProfile.objects.all()
...
def profile_view(request):
    ...
    return render(
        request, "management/accounts/profile.html", {"profile": profile, "perm": perm}
    )
...
def change_password(request, id):
    ...
    if request.method == "POST":
        ...
        return response
    return render(request, "management/accounts/change-password.html", {"id": id})
